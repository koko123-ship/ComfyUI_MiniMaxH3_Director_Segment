...
import { appendLocalLatentsRow, renderTwoVideoSlot } from "./minimax_image_batch.js";
...
/* fl2v 镜卡 220px 窄:两个 latent 槽改成紧凑小行、上下堆叠,不再用大 16:9 缩略图撑高镜卡。 */
.bd-fl2v-shot .bd-local-latents-row{grid-template-columns:1fr;gap:4px}
.bd-fl2v-shot .bd-local-latents-row .bd-batch-video{min-height:24px;flex-direction:row;align-items:center;justify-content:flex-start;gap:6px;padding:1px 6px;border-radius:4px}
.bd-fl2v-shot .bd-local-latents-row .bd-batch-video .bd-r2v-thumb{width:auto;height:auto;aspect-ratio:auto;background:none;border:none;flex:1;min-width:0;overflow:visible;display:flex;align-items:center}
.bd-fl2v-shot .bd-local-latents-row .bd-batch-video .bd-r2v-thumb .bd-r2v-ph{font-size:10px;color:#888;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%}
.bd-fl2v-shot .bd-local-latents-row .bd-batch-video:not(.has-video) .bd-r2v-meta{display:none}
.bd-fl2v-shot .bd-local-latents-row .bd-batch-video .bd-r2v-meta{flex-direction:row;align-items:center;gap:4px;min-width:0;padding-right:14px}
.bd-fl2v-shot .bd-local-latents-row .bd-batch-video .tag{font-size:10px;flex:none}
.bd-fl2v-shot .bd-local-latents-row .bd-batch-video .bd-latent-name{font-size:10px}
.bd-fl2v-shot .bd-local-latents-row .bd-batch-video .x{position:absolute;top:50%;right:3px;transform:translateY(-50%);font-size:11px}
...
export function newFl2vShot(overrides = {}) {
    ...
    return {
        ...
        // 本地素材:保留,避免重载后消失。
        ...(overrides.twoVideo && (overrides.twoVideo.videoFile || overrides.twoVideo.fileName)
            ? { twoVideo: overrides.twoVideo }
            : {}),
        ...(overrides.guideLatent && (overrides.guideLatent.videoFile || overrides.guideLatent.fileName)
            ? { guideLatent: overrides.guideLatent }
            : {}),
        ...(overrides.twoLatent && (overrides.twoLatent.videoFile || overrides.twoLatent.fileName)
            ? { twoLatent: overrides.twoLatent }
            : {}),
    ...
    const segs = shots.map((shot, i) => {
        ...
        const seg = {
            ...
            endImage: endImage
                ...
                : null,
            // 本地素材字段透传(shots 是源数据,后端 _seg_raw 优先读 segments,必须同步)。
            ...(shot.twoVideo && (shot.twoVideo.videoFile || shot.twoVideo.fileName)
                ? { twoVideo: shot.twoVideo }
                : {}),
            ...(shot.guideLatent && (shot.guideLatent.videoFile || shot.guideLatent.fileName)
                ? { guideLatent: shot.guideLatent }
                : {}),
            ...(shot.twoLatent && (shot.twoLatent.videoFile || shot.twoLatent.fileName)
                ? { twoLatent: shot.twoLatent }
                : {}),
        };
        cursor += fc;
        return seg;
...
function renderFl2vShotCards(editor) {
    ...
    shots.forEach((shot, i) => {
        ...
        `;
        // 本地素材:二采视频 + 上传段间引导latent + 本地二采latent,每镜时长下方。
        const twoSlot = document.createElement("div");
        twoSlot.className = "bd-fl2v-shot-row";
        renderTwoVideoSlot(twoSlot, shot, i, editor);
        card.appendChild(twoSlot);
        appendLocalLatentsRow(card, shot, i, editor);
...
export function buildFl2vPayloadFields(editor) {
    ...
    const shots = (editor.timeline.shots || []).map((s, i) => ({
        ...
        endImage: s.endImage
            ...
            : null,
        twoVideo: s.twoVideo && (s.twoVideo.videoFile || s.twoVideo.fileName)
            ? {
                videoFile: s.twoVideo.videoFile || "",
                fileName: s.twoVideo.fileName || "",
                type: s.twoVideo.type || "input",
                subfolder: s.twoVideo.subfolder || "",
            }
            : undefined,
        guideLatent: s.guideLatent && (s.guideLatent.videoFile || s.guideLatent.fileName)
            ? {
                videoFile: s.guideLatent.videoFile || "",
                fileName: s.guideLatent.fileName || "",
                type: s.guideLatent.type || "input",
                subfolder: s.guideLatent.subfolder || "",
            }
            : undefined,
        twoLatent: s.twoLatent && (s.twoLatent.videoFile || s.twoLatent.fileName)
            ? {
                videoFile: s.twoLatent.videoFile || "",
                fileName: s.twoLatent.fileName || "",
                type: s.twoLatent.type || "input",
                subfolder: s.twoLatent.subfolder || "",
            }
            : undefined,
    }));
    return {
