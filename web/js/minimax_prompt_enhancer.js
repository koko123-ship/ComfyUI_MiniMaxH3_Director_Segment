...
async function fetchImageB64(imageFile) {
    const resp = await api.fetchApi("/minimax/director_local/image_b64", {
        method: "POST",
...
export function mountPromptEnhancerPanel(editor, parentEl) {
    ...
    pe.fetchModels = async (silent = false) => {
        ...
        try {
            ...
            const resp = await api.fetchApi("/minimax/director_local/enhance_models", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
    ...
    pe.fetchTemplate = async (resetIfDefault = false) => {
        ...
        try {
            const resp = await api.fetchApi("/minimax/director_local/get_template", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    pe.collectVisionImagesForBlock = async (block, taskKey) => {
        ...
        if (videoFile && editor.getDirectorMode?.() === "video") {
            const resp = await api.fetchApi("/minimax/director_local/extract_frames", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    filename: videoFile,
    ...
    pe.callEnhanceApi = async (prompt, taskKey, block, cfg) => {
        ...
        const resp = await api.fetchApi("/minimax/director_local/enhance", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
    ...
    pe.unloadModel = async () => {
        ...
        try {
            const resp = await api.fetchApi("/minimax/director_local/unload_model", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    llm_url: llmUrl,
