...
/** 素材组(t2v/i2v/r2v 批量)默认 10s;fl2v 镜头与其他生成保持 5s。 */
export function defaultDurationSec(taskKey) {
    if (isImageBatchTask(taskKey)) return 0;
    if (isVideoBatchTask(taskKey)) return 10;
    if (FL2V_TASKS.has(taskKey)) return 5;
    return 5;
}
...
/** Master「段间衔接」truthy check (timeline.output). Mutually exclusive with
 * 「段间引导」 — callers must clear the other master when one turns on. */
export function isJoinMasterEnabled(output) {
    if (!output) return false;
