...
import { t, SETTINGS_STORAGE_KEY } from "./minimax_i18n.js";
...
/* 折叠导航:未选中组只显示头部一行,选中组展开主体。 */
.bd-batch-card.collapsed{padding:6px 10px;gap:4px;min-height:0;background:#151515;border-color:#2a2a2a;box-shadow:none;grid-template-columns:minmax(0,1fr);cursor:pointer}
.bd-batch-card.collapsed:hover{border-color:#555;background:#191919}
.bd-batch-card.collapsed .bd-batch-head{border-bottom:none;margin-bottom:0;padding-bottom:0;gap:10px}
.bd-batch-card.collapsed .bd-batch-head b{font-size:11px;color:#9ab}
.bd-batch-card.collapsed .bd-batch-head-meta{gap:10px}
.bd-batch-card.expanded{border-color:#4fff8f;box-shadow:0 0 0 1px rgba(79,255,143,.35)}
...
.bd-local-latents-row{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;width:100%}
.bd-local-latents-row .bd-batch-video.has-video{min-height:52px;justify-content:center}
.bd-local-latents-row .bd-batch-video .bd-latent-name{max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#8aa;font-size:10px;padding:0}
...
/* t2v/i2v/fl2v:本地二采视频预览盒 + 下方两个本地 latent 槽的容器。 */
.bd-batch-preview-wrap{display:flex;flex-direction:column;gap:7px;min-width:0;justify-content:center}
.bd-batch-preview-wrap .bd-batch-preview{width:100%;max-width:none;min-height:96px;justify-self:auto}
/* 窄预览列(宽视口)latent 槽纵向堆叠;窄视口卡片单列时改为左右并排。 */
.bd-batch-preview-wrap .bd-local-latents-row{grid-template-columns:1fr}
@media(max-width:720px){.bd-batch-preview-wrap .bd-local-latents-row{grid-template-columns:repeat(2,minmax(0,1fr))}}
`;
...
/** JSON position(从1起,对应显示「图片 N」)→ 内部 index(0基);越界返回 null。 */
function _posToIndex(pos, max) {
...
/** 自动填充:读设置里保存的服务器路径 → 服务端把 JSON 引用素材复制进 input/ → 追加为新 r2v 素材组。 */
export async function autoFillR2vSegmentsFromJson(editor) {
    if (!editor?.isR2vBatch?.()) return;
    const path = localStorage.getItem(SETTINGS_STORAGE_KEY);
    if (!path) {
        alert(t("settings.noPath"));
        return;
    }
    try {
        const resp = await api.fetchApi("/minimax/director_local/import_material", {
            method: "POST",
        ...
        if (!Array.isArray(data.segments) || !data.segments.length) {
            throw new Error((data.errors || ["服务器未返回任何段"]).join("\n"));
        }
        const appended = _appendSegmentsFromServer(editor, data);
        if (appended > 0) {
            normalizeImageBatchSegments(editor);
            editor.selectedIndex = Math.max(0, editor.timeline.segments.length - 1);
            editor.renderImageBatchGroups();
        ...
        if (data.errors?.length) {
            console.warn("[MiniMax H3Director] import skipped:", data.errors);
            alert(`已导入 ${appended} 段素材组;跳过 ${data.errors.length} 个问题:\n` + data.errors.slice(0, 12).join("\n"));
        }
    } catch (err) {
        console.error("[MiniMax H3Director] import failed:", err);
        alert("导入素材失败: " + (err?.message || err));
    }
}

/** 把服务端返回的 segments(素材已复制进 input/,imageFile/audioFile 就绪)追加到时间轴末尾。 */
function _appendSegmentsFromServer(editor, data) {
    const taskKey = resolveTaskKey(editor.getTaskKey?.() || editor.taskTypeWidget?.value);
    let appended = 0;
    for (const src of data.segments) {
        const seg = newBatchSegment({ durationSec: defaultDurationSec(taskKey), negativePrompt: "" });
        seg.prompt = src.prompt != null ? String(src.prompt) : "";
        seg.refs = [];
        seg.refAudios = [];
        for (const img of _sortByPosition(src.images)) {
            const index = _posToIndex(img?.position, MAX_REFERENCE_IMAGES);
            if (index == null || !img.imageFile) continue;
            seg.refs.push({ index, imageFile: img.imageFile, type: "input", subfolder: "" });
        }
        for (const au of _sortByPosition(src.audio)) {
            const index = _posToIndex(au?.position, MAX_REFERENCE_AUDIOS);
            if (index == null || !au.audioFile) continue;
            seg.refAudios.push({ index, audioFile: au.audioFile, fileName: au.fileName || au.audioFile, type: "input", subfolder: "" });
        }
        editor.timeline.segments.push(seg);
        appended++;
    }
    return appended;
}
...
export function deleteImageBatchGroup(editor, index) {
    ...
    // 先 flush:把 DOM 里每个 textarea 的草稿写回对应段(避免删除后按旧索引写错位,
    // 把被删组的提示词盖到下一个组上)。
    flushBatchPromptInputs(editor);
    editor.timeline.segments.splice(index, 1);
    // 同步移除对应 DOM 卡片,让后续 normalize/render 的 flush 读到与 segments 对齐的 DOM。
    const staleCard = editor.batchList?.children?.[index];
    staleCard?.remove?.();
    normalizeImageBatchSegments(editor);
    cleanupAfterSegmentDelete(editor, index);
    ...
    editor.renderImageBatchGroups();
    editor.commit();
    // 立即写 timeline_data,避免防抖导致已删素材仍留在运行时计划里。
    editor.flushTimelineSync?.();
    editor.updateVideoNameLabel?.();
    editor.updateDomWidgetHeight?.();
}

/** 删除素材组后清理历史残留:勾选索引重排、兜底清理。 */
export function cleanupAfterSegmentDelete(editor, deletedIndex) {
    const t = editor.timeline;
    if (!t) return;
    // 勾选索引重排:删掉 deletedIndex,之后的索引减 1。
    if (Array.isArray(t.runSelection) && t.runSelectEnabled) {
        t.runSelection = [...new Set(
            (t.runSelection || [])
...
/** 本地二采视频:上传视频存到 seg.twoVideo,节点输出 frames_0/audio_0 供外部采样用。 */
export function renderTwoVideoSlot(slotEl, seg, index, editor) {
    slotEl.classList.add("bd-two-video");
    slotEl.innerHTML = "";
    // 二采优先存 refVideos(index>=3,参考视频持久化最稳),兼容旧 twoVideo 字段。
    const two = (seg.refVideos || []).find((r) => Number(r.index ?? r.slot) >= 3)
        || (seg.twoVideo && (seg.twoVideo.videoFile || seg.twoVideo.fileName) ? seg.twoVideo : null);
    const file = two?.videoFile || two?.fileName || "";
    const hasMedia = !!file;

    // 参照 r2v 视频槽的样式:缩略图 + 播放 + 「本地二采」标签 + 移除。
    const slot = document.createElement("div");
    slot.className = `bd-batch-video${hasMedia ? " has-video" : ""}`;
    slot.dataset.twoVideo = "1";
    slot.title = hasMedia ? file : "上传本地二采视频(输出 frames_0 / audio_0)";
    ...
    if (hasMedia) {
        ...
        rm.onclick = (e) => {
            e.stopPropagation();
            if (Array.isArray(seg.refVideos)) {
                seg.refVideos = seg.refVideos.filter((r) => Number(r.index ?? r.slot) < 3);
            }
            seg.twoVideo = {};
            const live = (editor.timeline.segments || []).find((s) => s?.id && s.id === seg.id)
                || editor.timeline.segments?.[index]
                || editor.timeline.shots?.[index];
            if (live && live !== seg) {
                if (Array.isArray(live.refVideos)) {
                    live.refVideos = live.refVideos.filter((r) => Number(r.index ?? r.slot) < 3);
                }
                live.twoVideo = {};
            }
        ...
        plus.className = "bd-r2v-ph";
        plus.textContent = "＋ 本地二采";
        thumb.appendChild(plus);
    }

    slot.onclick = (e) => {
        e.stopPropagation();
        if (e.target.closest?.("button, video, .bd-r2v-play")) return;
        uploadSegTwoVideo(editor, index, seg);
    };
    slotEl.appendChild(slot);
}

export async function uploadSegTwoVideo(editor, index, segArg) {
    const segId = segArg?.id || editor.timeline.segments[index]?.id;
    pickFile("video/*,.mp4,.webm,.mov,.mkv,.avi", async (file) => {
        try {
            const uploaded = await uploadMedia(file);
            const videoFile = relPath(uploaded);
            if (!videoFile) throw new Error("Upload returned empty filename");
            const seg = segArg
                || (editor.timeline.segments || []).find((s) => s?.id && s.id === segId)
                || editor.timeline.segments?.[index]
                || editor.timeline.shots?.[index];
            if (!seg) return;
            const two = {
                videoFile,
                fileName: uploaded.name || uploaded.filename || videoFile,
                subfolder: uploaded.subfolder || "",
                type: "input",
            };
            // 主存 refVideos(index=5,参考视频持久化最稳);兼容字段 twoVideo 也写一份。
            if (Array.isArray(seg.refVideos)) {
                seg.refVideos = [
                    ...seg.refVideos.filter((r) => Number(r.index ?? r.slot) < 3),
                    { index: 5, kind: "two", ...two },
                ];
            } else {
                seg.refVideos = [{ index: 5, kind: "two", ...two }];
            }
            seg.twoVideo = two;
            editor.scheduleRender?.();
            editor.commit(false, { syncTimeline: true });
        } catch (err) {
            console.error("[MiniMax H3Director] two-video upload failed:", err);
            alert(t("upload.alertFailed", { err: err?.message || err }));
        }
    });
}
...
/**
 * 上传本地 latent 文件(.pt/.bin/.ckpt/.safetensors)。
 * field 为 "guideLatent"(上传段间引导latent,替代接口 prev_latent)或
 * "twoLatent"(本地二采latent,纯透传到输出 latent_0)。
 */
export async function uploadLocalLatent(editor, index, segArg, field) {
    const segId = segArg?.id || editor.timeline.segments[index]?.id;
    pickFile(".pt,.bin,.ckpt,.safetensors", async (file) => {
        try {
            const uploaded = await uploadMedia(file);
            const latentFile = relPath(uploaded);
            if (!latentFile) throw new Error("Upload returned empty filename");
            const seg = segArg
                || (editor.timeline.segments || []).find((s) => s?.id && s.id === segId)
                || editor.timeline.segments?.[index]
                || editor.timeline.shots?.[index];
            if (!seg) return;
            seg[field] = {
                videoFile: latentFile,
                fileName: uploaded.name || uploaded.filename || latentFile,
                subfolder: uploaded.subfolder || "",
                type: "input",
            };
            editor.scheduleRender?.();
            ...
            console.error("[MiniMax H3Director] local latent upload failed:", err);
            alert(t("upload.alertFailed", { err: err?.message || err }));
        }
    });
}

/** 渲染本地 latent 槽(无预览,只显示标签 + 文件名 + 移除)。 */
export function renderLocalLatentSlot(slotEl, seg, index, editor, field, label) {
    slotEl.classList.add("bd-two-video");
    slotEl.innerHTML = "";
    const ref = seg[field];
    ...
    const slot = document.createElement("div");
    slot.className = `bd-batch-video${hasMedia ? " has-video" : ""}`;
    slot.dataset.latentField = field;
    slot.title = hasMedia ? file : `上传${label}(.pt/.safetensors)`;
    const meta = document.createElement("div");
    meta.className = "bd-r2v-meta";
    const tag = document.createElement("span");
    tag.className = "tag";
    ...
    slot.onclick = (e) => {
        e.stopPropagation();
        if (e.target.closest?.("button, video, .bd-r2v-play")) return;
        uploadLocalLatent(editor, index, seg, field);
    };
    slotEl.appendChild(slot);
}

/** 本地 latent 行:上传段间引导latent + 本地二采latent,一左一右,放在本地素材槽下方。 */
export function appendLocalLatentsRow(container, seg, index, editor) {
    const latentRow = document.createElement("div");
    latentRow.className = "bd-local-latents-row";
    const guideSlot = document.createElement("div");
    guideSlot.className = "bd-two-video-slot";
    renderLocalLatentSlot(
        guideSlot, seg, index, editor, "guideLatent",
        t("batch.guideLatent.label") || "上传段间引导latent",
    );
    const twoLatentSlot = document.createElement("div");
    twoLatentSlot.className = "bd-two-video-slot";
    renderLocalLatentSlot(
        twoLatentSlot, seg, index, editor, "twoLatent",
        t("batch.twoLatent.label") || "本地二采latent",
    );
    latentRow.append(guideSlot, twoLatentSlot);
    container.appendChild(latentRow);
}
...
function appendR2vMediaSections(card, seg, index, editor) {
    ...
    // 本地素材:二采视频 + 上传段间引导latent + 本地二采latent,放在音频槽下方。
    const twoSection = createR2vSection(
        t("batch.twoVideo.label") || "本地",
        seg.twoVideo?.videoFile || seg.twoVideo?.fileName ? "1" : "0/1",
    );
    const twoSlot = document.createElement("div");
    twoSlot.className = "bd-two-video-slot";
    renderTwoVideoSlot(twoSlot, seg, index, editor);
    twoSection.appendChild(twoSlot);
    // 两个 latent 槽:一左一右,在二采视频槽下方。
    appendLocalLatentsRow(twoSection, seg, index, editor);
    assets.appendChild(twoSection);
...
export function renderImageBatchGroups(editor) {
    ...
    editor.timeline.segments.forEach((seg, index) => {
        ...
        card.onclick = (e) => {
            ...
            // 折叠导航:单击导航栏(折叠组头部)即重建卡片列表,展开该组主体,其余组保持折叠。
            editor.renderImageBatchGroups?.();
            editor.scheduleRender?.();
            editor.updateVideoNameLabel?.();
        };
        ...
        // Per-segment continuity / join (master「段间引导」or「段间衔接」; skip segment 1).
        const masterCont = isContinuityMasterEnabled(editor.timeline?.output)
            || isJoinMasterEnabled(editor.timeline?.output);
        ...
        // 折叠导航:未选中的组只渲染头部一行「素材N 5s」,选中组才展开主体,减少 DOM。
        card.classList.toggle("collapsed", index !== editor.selectedIndex);
        card.classList.toggle("expanded", index === editor.selectedIndex);
        ...
        if (isR2v) {
            ...
            // t2v / i2v / fl2v:预览区为「本地二采视频」上传位。
            renderTwoVideoSlot(preview, seg, index, editor);
        }

        // 本地 latent 槽(上传段间引导latent + 本地二采latent)放预览盒外的独立容器:
        // 采样时预览盒会被清空重挂(live preview),放进盒内会被一并抹掉。
        let previewSlot = preview;
        if (!isR2v) {
            const previewWrap = document.createElement("div");
            previewWrap.className = "bd-batch-preview-wrap";
            previewWrap.appendChild(preview);
            appendLocalLatentsRow(previewWrap, seg, index, editor);
            previewSlot = previewWrap;
        }
...
export function setImageBatchPreview(editor, segmentIndex, imageB64, extra = {}) {
    // 折叠导航:实时采样时自动展开对应组,让实时预览可见(仅首次切换时重渲染)。
    if (extra.live && editor.selectedIndex !== segmentIndex) {
        editor.selectedIndex = segmentIndex;
        editor.renderImageBatchGroups();
    }
...
export function getImageBatchUiHeight(editor) {
    ...
    const key = resolveTaskKey(editor?.getTaskKey?.() || editor?.taskTypeWidget?.value);
    // 折叠导航:只展开选中组(完整高度),其余为一行折叠标签(矮)。
    const fullRowH = key === "r2v" ? 420 : (isVideoBatchTask(key) ? 155 : 130);
    const collapsedRowH = 36;
    const listContentH = (n - 1) * collapsedRowH + fullRowH + Math.max(0, n - 1) * BATCH_LIST_GAP;
...
export function setR2vToolbar(editor, enabled) {
    ...
    const fillBtn = editor.root?.querySelector('[data-a="r2v-auto-fill"]');
    if (fillBtn) {
        fillBtn.classList.toggle("hidden", !enabled || externalLocked);
        fillBtn.disabled = !enabled || externalLocked;
    }
...
export function updateR2vToolbarBtns(editor) {
    ...
    if (fillBtn) {
        fillBtn.classList.toggle("hidden", !show);
        fillBtn.disabled = !show;
    }
}
