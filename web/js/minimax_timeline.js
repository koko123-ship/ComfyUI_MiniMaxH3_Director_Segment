...
/** 段间衔接 (join) is opt-in; default off unless explicitly enabled in output. */
function isJoinEnabled(output) {
    if (!output) return false;
    const raw = output.joinEnabled ?? output.join_enabled;
    if (raw === true || raw === 1) return true;
    if (typeof raw === "string") {
        const s = raw.trim().toLowerCase();
        return s === "true" || s === "1" || s === "yes" || s === "on";
...
/** 稀疏取帧 for the join row; default off unless explicitly enabled in output. */
function isSparseJoinEnabled(output) {
    if (!output) return false;
    const raw = output.joinSparseContext ?? output.join_sparse_context;
    if (raw === true || raw === 1) return true;
    if (typeof raw === "string") {
        const s = raw.trim().toLowerCase();
        return s === "true" || s === "1" || s === "yes" || s === "on";
...
const CONTINUITY_FRAME_CHOICES = [5, 22, 39, 56, 73, 90, 107, 124, 141, 158, 175, 192, 209, 226];
 ...
 * 17*21+5 = 362 ≈ 15.1s @24fps). 段间引导 keeps CONTINUITY_FRAME_CHOICES ≤226.
 */
const JOIN_FRAME_CHOICES = [
...
function snapJoinFrames(raw) {
    const n = parseInt(raw, 10);
    const value = Number.isFinite(n) ? n : DEFAULT_CONTINUITY_FRAMES;
    let best = DEFAULT_CONTINUITY_FRAMES;
    let bestDist = Infinity;
    for (const choice of JOIN_FRAME_CHOICES) {
        const dist = Math.abs(choice - value);
        if (dist < bestDist || (dist === bestDist && choice > best)) {
            best = choice;
            bestDist = dist;
...
/** <option> markup for the 上下文帧数 dropdown (段间引导 only, grid capped at 226). */
function continuityOverlapOptionsHtml(selected = DEFAULT_CONTINUITY_FRAMES) {
    const sel = snapContinuityFrames(selected);
    return CONTINUITY_FRAME_CHOICES
        .map((choice) => `<option value="${choice}"${choice === sel ? " selected" : ""}>${choice}</option>`)
        .join("\n");
}

/** <option> markup for the 段间衔接 dropdown — join-only extended reach (~15s). */
function joinOverlapOptionsHtml(selected = DEFAULT_CONTINUITY_FRAMES) {
    const sel = snapJoinFrames(selected);
    return JOIN_FRAME_CHOICES
        .map((choice) => `<option value="${choice}"${choice === sel ? " selected" : ""}>${choice}</option>`)
        .join("\n");
}
...
function normalizeOutputContinuity(output = {}) {
    ...
    const rawJoin = output.joinOverlapFrames ?? output.join_overlap_frames ?? DEFAULT_CONTINUITY_FRAMES;
    const contOn = isContinuityEnabled(output);
    const joinOn = isJoinEnabled(output);
    // 稀疏取帧 is only meaningful while 段间衔接 is on; keep the stored value
    // (join-off / continuity-on reads it as off through the gate below).
    const sparseOn = isJoinEnabled(output)
        && !contOn
        && isSparseJoinEnabled(output);
    // Mutual exclusion: backend rejects both-on; normalize so continuity wins.
    return {
        ...output,
        continuityEnabled: contOn,
        continuityOverlapFrames: snapContinuityFrames(rawOverlap),
        joinEnabled: contOn ? false : joinOn,
        joinOverlapFrames: snapJoinFrames(rawJoin),
        joinSparseContext: sparseOn,
...
function stripTimelineContinuityRootFields(timeline) {
    ...
    delete timeline.joinEnabled;
    delete timeline.join_enabled;
    delete timeline.joinOverlapFrames;
    delete timeline.join_overlap_frames;
    delete timeline.joinSparseContext;
    delete timeline.join_sparse_context;
}
...
function sanitizeSegmentForPayload(seg) {
    ...
    return {
        ...
        referenceVideo: rest.referenceVideo
            ...
            : undefined,
        // 本地二采视频:显式保留,避免被清理。
        twoVideo: rest.twoVideo
            ? {
                videoFile: rest.twoVideo.videoFile || "",
                fileName: rest.twoVideo.fileName || "",
                type: rest.twoVideo.type || "input",
                subfolder: rest.twoVideo.subfolder || "",
            }
            : undefined,
        // 上传段间引导latent / 本地二采latent:显式保留,避免被清理。
        guideLatent: rest.guideLatent
            ? {
                videoFile: rest.guideLatent.videoFile || "",
                fileName: rest.guideLatent.fileName || "",
                type: rest.guideLatent.type || "input",
                subfolder: rest.guideLatent.subfolder || "",
            }
            : undefined,
        twoLatent: rest.twoLatent
            ? {
                videoFile: rest.twoLatent.videoFile || "",
                fileName: rest.twoLatent.fileName || "",
                type: rest.twoLatent.type || "input",
                subfolder: rest.twoLatent.subfolder || "",
            }
            : undefined,
    };
}
...
.bd-text{background:#181818;border:1px solid #333;border-radius:4px;color:#eee;padding:6px 8px;font-size:11px;width:100%;box-sizing:border-box}
.bd-settings-row{display:flex;flex-direction:column;gap:4px}
.bd-settings-row label{color:#888;font-size:10px;white-space:nowrap}
...
.bd-ref-size-row{display:inline-flex;align-items:center;gap:4px;min-width:0}
.bd-ref-size-row .bd-select{max-width:118px;flex-shrink:1}
${IMAGE_BATCH_STYLES}
${FL2V_STYLES}
...
async function uploadVideoChunked(file, onProgress) {
    ...
    for (let i = 0; i < totalChunks; i++) {
        ...
        const resp = await api.fetchApi("/minimax/director_local/upload_chunk", { method: "POST", body });
        if (!resp.ok) {
            const text = await resp.text();
...
/** 防御:节点高度超过「内容高度或此上限」就收敛,防止加素材/尺寸循环把画布撑爆。 */
const DIRECTOR_UI_MAX_ABS_H = 3200;

function clampDirectorNodeSize(node, editor) {
    if (!node?.size || !node.computeSize) return false;
    bindDomWidgetContentComputeSize(editor);
    const ideal = node.computeSize?.()?.[1];
    if (ideal == null) return false;
    const cap = Math.max(ideal, DIRECTOR_UI_MAX_ABS_H);
    const curH = node.size[1] || 0;
    if (curH > cap) {
        node.setSize([node.size[0], cap]);
        node.setDirtyCanvas?.(true, true);
        return true;
    }
    return false;
...
function parseTimeline(raw, totalFrames, fps) {
    ...
    const base = {
        ...
        output: {
            ...
            joinEnabled: false, joinOverlapFrames: DEFAULT_CONTINUITY_FRAMES,
            joinSparseContext: false,
        },
    ...
    try {
        ...
        data.output = normalizeOutputContinuity({
            ...
            joinEnabled: data.output?.joinEnabled ?? data.output?.join_enabled,
            joinOverlapFrames: data.output?.joinOverlapFrames ?? data.output?.join_overlap_frames,
            // 稀疏取帧标记:随 timeline 序列化保留,切标签/重解析时不得回退(否则复选框被重置)。
            joinSparseContext: data.output?.joinSparseContext ?? data.output?.join_sparse_context,
            // r2v 参考图尺寸(统一设置):随 timeline 序列化保留,切标签/重解析时不回退 match。
            refImageSize: data.output?.refImageSize ?? data.output?.ref_image_size ?? undefined,
        });
...
class MiniMaxH3DirectorEditor {
    ...
    updateDomWidgetHeight(opts = {}) {
        ...
        if (!this.isPlaying) {
            ...
            // 防御:超上限就收敛,防止加素材/尺寸循环把画布撑爆导致卡顿。
            clampDirectorNodeSize(this.node, this);
        }
            ...
            return {
                ...
                segments: this.timeline.segments.map((s, i) => {
                    ...
                    return {
                        ...
                        // 本地二采视频 / 上传段间引导latent / 本地二采latent:显式保留,避免被清理。
                        twoVideo: clean.twoVideo || undefined,
                        guideLatent: clean.guideLatent || undefined,
                        twoLatent: clean.twoLatent || undefined,
    ...
    buildDOM() {
        ...
        toolbarWrap.innerHTML = `
            <div class="bd-toolbar">
                <div class="bd-actions">
                    ...
                    <button type="button" class="bd-btn hidden" data-a="r2v-auto-fill" data-i18n="toolbar.autoFill" data-i18n-title="tooltip.autoFill">导入素材</button>
                ...
                <div class="bd-right">
                    <button type="button" class="bd-btn" data-a="settings" data-i18n="toolbar.settings" data-i18n-title="tooltip.settings">设置</button>
        ...
        outputBar.innerHTML = `
            ...
            <span class="bd-ref-size-row hidden" data-r="global-ref-size-row" data-i18n-title="panel.refImageSizeTooltip">
                <label data-i18n="panel.refImageSize">参考图尺寸</label>
                <select class="bd-select" data-r="global-ref-size">
            ...
            <span class="bd-continuous-ref hidden" data-r="segment-join-wrap" hidden aria-hidden="true" title="">
                <label><input type="checkbox" data-r="segment-join-cb"><span data-i18n="output.segmentJoin">段间衔接</span></label>
                <span class="bd-meta" data-i18n="output.joinOverlap">上下文帧数</span>
                <select class="bd-num" data-r="segment-join-overlap" style="width:72px">
                    ${joinOverlapOptionsHtml()}
                </select>
                <label class="bd-join-sparse hidden" data-r="segment-join-sparse-wrap" hidden aria-hidden="true">
                    <input type="checkbox" data-r="segment-join-sparse-cb"
                           data-i18n-title="tooltip.joinSparse"
                           title="尾部 6 步连续 + 往前每约 1s 取 1 步(场景一致性),采样更快。不勾选=整段密集上下文。"><span data-i18n="output.joinSparse">稀疏</span>
                </label>
            </span>
        ...
        this.globalRefSizeRow = this.root.querySelector('[data-r="global-ref-size-row"]');
        this.globalRefSize = this.root.querySelector('[data-r="global-ref-size"]');
        ...
        this.segmentJoinWrap = this.root.querySelector('[data-r="segment-join-wrap"]');
        this.segmentJoinCb = this.root.querySelector('[data-r="segment-join-cb"]');
        this.segmentJoinOverlap = this.root.querySelector('[data-r="segment-join-overlap"]');
        this.segmentJoinSparseWrap = this.root.querySelector('[data-r="segment-join-sparse-wrap"]');
        this.segmentJoinSparseCb = this.root.querySelector('[data-r="segment-join-sparse-cb"]');
    ...
    bindEvents() {
        ...
        bind('[data-a="r2v-auto-fill"]', () => { void autoFillR2vSegmentsFromJson(this); });
        ...
        if (this.segmentContinuityCb) {
            this.segmentContinuityCb.onchange = () => {
                ...
                if (on && this.segmentJoinCb) this.segmentJoinCb.checked = false;
                this.onOutputField("continuityEnabled", on);
                this.updateSegmentContinuityUI();
            };
        }
        ...
        if (this.segmentJoinCb) {
            this.segmentJoinCb.onchange = () => {
                const on = !!this.segmentJoinCb.checked;
                if (on && this.segmentContinuityCb) this.segmentContinuityCb.checked = false;
                this.onOutputField("joinEnabled", on);
                this.updateSegmentContinuityUI();
            };
        }
        if (this.segmentJoinOverlap) {
            const applyJoinOverlap = () => this.onOutputField("joinOverlapFrames", +this.segmentJoinOverlap.value);
            this.segmentJoinOverlap.onchange = applyJoinOverlap;
            this.segmentJoinOverlap.oninput = applyJoinOverlap;
            this.segmentJoinOverlap.addEventListener("keydown", (e) => e.stopPropagation());
            this.segmentJoinOverlap.addEventListener("keyup", (e) => e.stopPropagation());
        }
        if (this.segmentJoinSparseCb) {
            this.segmentJoinSparseCb.onchange = () => {
                this.onOutputField("joinSparseContext", !!this.segmentJoinSparseCb.checked);
                this.updateSegmentContinuityUI();
            };
        }
        ...
        if (this.globalRefSize) {
            // r2v 参考图尺寸:match=匹配画布,max=高清保真。存 output,全段统一。
            this.globalRefSize.onchange = () => {
                this.timeline.output = {
                    ...(this.timeline.output || {}),
                    refImageSize: this.globalRefSize.value === "max" ? "max" : "match",
                };
                this.commit(true);
            };
            this.globalRefSizeRow?.setAttribute(
                "title",
                t("panel.refImageSizeTooltip"),
            );
        }
        ...
        this.canvas.addEventListener("dblclick", (e) => {
            ...
            // 素材组批量模式（t2v/i2v/r2v 批量）：双击直接展开对应素材组，
            // 不再拆分出新的素材组（选中与否都直接展开）。
            if (this.isImageBatch()) {
                stopDomEvent(e);
                e.preventDefault();
                const { x, y } = this.getMousePos(e);
    ...
    updateReferenceImageVisibility({ hideTimeline = false, seg = null } = {}) {
        ...
        // r2v 参考图尺寸下拉:仅参考图任务显示,不受公共参数面板开关影响。
        this.globalRefSizeRow?.classList.toggle("hidden", !taskUsesReferenceImages(globalKey));
        if (this.globalRefSize) {
            this.globalRefSize.value = (this.timeline?.output?.refImageSize === "max") ? "max" : "match";
        }
    ...
    /** MiniMaxH3Segment / MiniMaxH3R2VSegment: 采样/解码/导出全在节点外,导出相关控件不显示。 */
    isR2vSegmentNode() {
        const cls = this.node?.comfyClass || this.node?.type || "";
        return cls === "MiniMaxH3R2VSegment" || cls === "MiniMaxH3Segment";
    }
    ...
    applyTaskLayout(prevMode) {
        ...
        // R2V Segment 节点采样/解码/导出全在外部:声音、导出方式、最大帧数都不显示。
        const isR2vSeg = this.isR2vSegmentNode?.() || false;
        this.outAudioWrap?.classList.toggle("hidden", !isVideoEditTask || isR2vSeg);
        if (this.outExportMode) {
            const hideExport = isR2vSeg || ((isBatch || isFl2v) && !showBatchExport);
            this.outExportMode.disabled = hideExport;
            this.outExportMode.classList.toggle("hidden", hideExport);
            this.outExportMode.previousElementSibling?.classList.toggle("hidden", hideExport);
        }
        if (this.outMaxFrames) {
            const hideMax = isR2vSeg || ((isBatch || isFl2v) && !showBatchExport);
            this.outMaxFrames.disabled = hideMax;
            this.outMaxFrames.classList.toggle("hidden", hideMax);
            this.outMaxFrames.previousElementSibling?.classList.toggle("hidden", hideMax);
        }
    ...
    syncOutputUIFromTimeline() {
        ...
        if (this.segmentJoinCb) this.segmentJoinCb.checked = isJoinEnabled(out);
        if (this.segmentJoinOverlap) {
            this.segmentJoinOverlap.value = String(
                snapJoinFrames(out.joinOverlapFrames ?? DEFAULT_CONTINUITY_FRAMES),
            );
        }
        if (this.segmentJoinSparseCb) {
            this.segmentJoinSparseCb.checked = isJoinEnabled(out) && isSparseJoinEnabled(out);
        }
    ...
    updateSegmentContinuityUI() {
        ...
        if (this.segmentJoinWrap) {
            this.segmentJoinWrap.classList.toggle("hidden", !show);
            this.segmentJoinWrap.hidden = !show;
            this.segmentJoinWrap.setAttribute("aria-hidden", show ? "false" : "true");
            this.segmentJoinWrap.title = show
                ? t("tooltip.segmentJoin")
                : "";
        }
        const joinOn = show && this.segmentJoinCb ? this.segmentJoinCb.checked : false;
        if (this.segmentJoinSparseWrap) {
            // 稀疏取帧 lives only inside the join row while 段间衔接 is on.
            const showSparse = !!joinOn;
            this.segmentJoinSparseWrap.classList.toggle("hidden", !showSparse);
            this.segmentJoinSparseWrap.hidden = !showSparse;
            this.segmentJoinSparseWrap.setAttribute(
                "aria-hidden",
                showSparse ? "false" : "true",
            );
        }
        ...
        if (this.segmentJoinOverlap && this.timeline?.output) {
            const frames = snapJoinFrames(
                this.timeline.output.joinOverlapFrames ?? DEFAULT_CONTINUITY_FRAMES,
            );
            this.segmentJoinOverlap.value = String(frames);
            this.timeline.output.joinOverlapFrames = frames;
        }
        ...
        if (this.segmentJoinCb && this.timeline?.output) {
            // Keep DOM aligned with timeline; eligibility only gates visibility.
            this.segmentJoinCb.checked = isJoinEnabled(this.timeline.output);
        }
        if (this.segmentJoinSparseCb && this.timeline?.output) {
            this.segmentJoinSparseCb.checked =
                isJoinEnabled(this.timeline.output)
                && isSparseJoinEnabled(this.timeline.output);
        }
    ...
    syncSegmentContinuityFromPrevUI() {
        ...
        const masterOn = isContinuityEligible(this)
            && (isContinuityMasterEnabled(this.timeline?.output)
                || isJoinMasterEnabled(this.timeline?.output));
    ...
    onOutputField(key, value) {
        ...
        if (key === "aspectRatio") {
            ...
            if (value) this.timeline.output.joinEnabled = false;
        } else if (key === "continuityOverlapFrames") {
            this.timeline.output.continuityOverlapFrames = snapContinuityFrames(value);
        } else if (key === "joinEnabled") {
            this.timeline.output.joinEnabled = !!value;
            if (value) this.timeline.output.continuityEnabled = false;
            if (!value) {
                this.timeline.output.joinSparseContext = false;
                if (this.segmentJoinSparseCb) this.segmentJoinSparseCb.checked = false;
            }
        } else if (key === "joinOverlapFrames") {
            this.timeline.output.joinOverlapFrames = snapJoinFrames(value);
        } else if (key === "joinSparseContext") {
            this.timeline.output.joinSparseContext = !!value;
        }
        this.syncOutputUIFromTimeline();
        if (this.isFl2vMode()) updateFl2vDetailUI(this);
        // Refresh per-segment「引用上段」checkboxes when master toggle changes.
        if (key === "continuityEnabled" || key === "joinEnabled") {
            if (this.isImageBatch()) this.renderImageBatchGroups?.();
            this.syncSegmentContinuityFromPrevUI?.();
        }
        this.commit();
    ...
    syncOutputToWidgets() {
        ...
        this.timeline.output = {
            ...
            joinEnabled: isJoinEnabled(prevOut) && !isContinuityEnabled(prevOut),
            joinOverlapFrames: snapJoinFrames(
                prevOut.joinOverlapFrames ?? DEFAULT_CONTINUITY_FRAMES,
            ),
            joinSparseContext: isJoinEnabled(prevOut)
                && !isContinuityEnabled(prevOut)
                && isSparseJoinEnabled(prevOut),
        };
        if (this.widthWidget) this.widthWidget.value = resolved.width;
        if (this.heightWidget) this.heightWidget.value = resolved.height;
        if (this.refMaxWidget) this.refMaxWidget.value = resolved.refMaxSize;
    ...
    syncFromWidgets() {
        ...
        if (continuityEligible && this.segmentJoinCb) {
            this.timeline.output.joinEnabled = !!this.segmentJoinCb.checked;
        } else {
            this.timeline.output.joinEnabled = isJoinEnabled(this.timeline.output);
        }
        // Mutual exclusion: continuity wins.
        if (this.timeline.output.continuityEnabled) this.timeline.output.joinEnabled = false;
        // 稀疏取帧 reads the DOM checkbox only while 段间衔接 is actually on.
        if (continuityEligible && this.segmentJoinSparseCb && this.timeline.output.joinEnabled) {
            this.timeline.output.joinSparseContext = !!this.segmentJoinSparseCb.checked;
        } else {
            this.timeline.output.joinSparseContext =
                !!this.timeline.output.joinEnabled && isSparseJoinEnabled(this.timeline.output);
        }
        ...
        if (continuityEligible && this.segmentJoinOverlap) {
            this.timeline.output.joinOverlapFrames = snapJoinFrames(
                this.segmentJoinOverlap.value
                    ?? this.timeline.output.joinOverlapFrames
                    ?? DEFAULT_CONTINUITY_FRAMES,
            );
        } else if (this.timeline.output.joinOverlapFrames != null) {
            this.timeline.output.joinOverlapFrames = snapJoinFrames(
                this.timeline.output.joinOverlapFrames,
            );
        }
    ...
    openSettingsPanel() {
        ...
        panel.innerHTML = `
            ...
            <div class="bd-settings-body"></div>
            <div class="bd-modal-actions"></div>`;
        panel.querySelector(".bd-modal-title").textContent = t("toolbar.settings");

        const bodyEl = panel.querySelector(".bd-settings-body");
        const row = document.createElement("div");
        row.className = "bd-settings-row";
        const label = document.createElement("label");
        label.textContent = t("settings.materialPath");
        const input = document.createElement("input");
        input.type = "text";
        input.className = "bd-text";
        input.placeholder = t("settings.pathPlaceholder");
        input.value = localStorage.getItem(SETTINGS_STORAGE_KEY) || "";
        ...
        const finish = (value) => {
            ...
            if (typeof value === "string") {
                try { localStorage.setItem(SETTINGS_STORAGE_KEY, value); } catch (_) { /* ignore */ }
            }
        };
    ...
    async probeVideoFile(relPath, subfolder = "", type = "input") {
        const resp = await api.fetchApi("/minimax/director_local/probe_video", {
            method: "POST",
    ...
    async smartSplit() {
        ...
        try {
            ...
            const resp = await api.fetchApi("/minimax/director_local/detect_shots", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    clips,
    ...
    _drawTimelineCanvas(width) {
        ...
        for (let i = 0; i < segs.length; i++) {
            ...
            // r2v 素材组:时间轴只显示素材图,不叠加提示词黑条(提示词在卡片里看)。
            if (!this.isR2vBatch() && (!this.isFl2vMode() || seg.isStartFrame)) {
...
const EXTERNAL_GROUP_NODE_TYPES = new Set([
    "MiniMaxH3DirectorGroupImageToVideoLocal",
    "MiniMaxH3DirectorGroupReferenceToVideoLocal",
]);
const EXTERNAL_COMBINE_NODE_TYPE = "MiniMaxH3DirectorGroupsCombineLocal";
...
function readExternalGroupSpec(node, graph = null) {
    ...
    let refVideos = [];
    let refAudios = [];
    if (cls === "MiniMaxH3DirectorGroupReferenceToVideoLocal") {
...
function isMiniMaxH3DirectorNode(node) {
    const cls = node?.comfyClass || node?.type || "";
    return cls === "MiniMaxH3DirectorLocal" || cls === "ComfyMiniMaxH3DirectorLocal"
        || cls === "MiniMaxH3R2VSegment" || cls === "MiniMaxH3Segment";
}
...
app.registerExtension({
    name: "ComfyUI.MiniMaxH3DirectorPluginLocal",
    async setup() {
        ...
        // ComfyUI 全局粘贴只跳过 input/textarea,不认 contenteditable → 导演台提示词框
        // (@ 引用用的 contenteditable)粘贴时会被 ComfyUI 连内部复制的节点一起粘进来。
        // 捕获阶段拦截:目标是导演台编辑器内的 contenteditable 时,自己插文本、阻止 ComfyUI 节点粘贴。
        document.addEventListener("paste", (e) => {
            const t = e.target;
            if (!t || t instanceof HTMLInputElement || t instanceof HTMLTextAreaElement) return;
            if (!(t.isContentEditable && (t.closest?.(".mmx-host") || t.closest?.(".bd-wrap")))) return;
            e.preventDefault();
            ...
            // 触发 input,让 @ 引用编辑器 rehydrate。
            try { t.dispatchEvent(new InputEvent("input", { bubbles: true })); } catch (_) { /* ignore */ }
        }, true);
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        const onDeselect = nodeType.prototype.onDeselect;
        nodeType.prototype.onDeselect = function () {
            const out = onDeselect?.apply(this, arguments);
            // 失去选中时让编辑器内的输入框失焦,避免 Ctrl+V 粘进"最后点过的节点"。
            const active = typeof document !== "undefined" ? document.activeElement : null;
            if (active && this._minimaxDomWidget?.element?.contains?.(active)) {
                active.blur?.();
            }
            return out;
        };
