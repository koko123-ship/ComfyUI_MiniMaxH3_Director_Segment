...
def segment_cache_fingerprint(seg: SegmentPlan, plan: DirectorPlan) -> dict[str, Any]:
    ...
    return {
        ...
        "join": plan.join_enabled,
        "join_overlap": plan.join_overlap_frames if plan.join_enabled else 0,
    }
...
def prune_stale_segment_cache(node_id, valid_count):
    """删除已删素材的磁盘缓存残留(seg_{idx} 且 idx >= valid_count)。绝不抛异常。"""
    try:
        root = _cache_root(node_id)
        ...
        for p in root.iterdir():
            ...
            if not name.startswith("seg_"):
                continue
            try:
                num = int(name[len("seg_"):].split(".", 1)[0])
            except (ValueError, IndexError):
                continue
            if num >= int(valid_count):
                if _safe_unlink(p):
                    removed += 1
        return removed
    except Exception as exc:
        log.warning("MiniMax H3 Segment: 清理历史缓存失败: %s", exc)
        return 0
