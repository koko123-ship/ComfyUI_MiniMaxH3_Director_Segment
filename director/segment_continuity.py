...
def resolve_join_settings(timeline: dict, *, segment_count: int) -> tuple[bool, int]:
    """Read 段间衔接 (join) master flags from timeline JSON (output only; default off).

    Mutually exclusive with 段间引导 — callers must reject both on (see
    ``build_director_plan`` and the timeline builders). Join reach snaps on the
    join-only extended grid (~15s); 段间引导 keeps ``snap_context_frames``.
    """
    if segment_count < 2:
        return False, 0
    output = timeline.get("output") or {}
    enabled = _truthy_continuity_flag(
        output.get("joinEnabled", output.get("join_enabled"))
    )
    if not enabled:
        return False, 0
    raw = (
        output.get("joinOverlapFrames")
        or output.get("join_overlap_frames")
        or DEFAULT_CONTINUITY_OVERLAP
    )
    return True, snap_join_context_frames(raw)


def resolve_join_sparse_context(timeline: dict) -> bool:
    """稀疏取帧 checkbox for the join row (output only; default off)."""
    output = timeline.get("output") or {}
    return _truthy_continuity_flag(
        output.get("joinSparseContext", output.get("join_sparse_context"))
    )
...
def is_join_active(plan: DirectorPlan, seg: SegmentPlan) -> bool:
    """True only when UI「段间衔接」is ON and this segment should join the previous.

    Like 段间引导, but negative-time: the previous tail is pinned into
    conditioning BEFORE frame 0 and the output latent is never extended.
    Supported tasks and per-segment「引用上段」gating match 段间引导.
    """
    return (
        getattr(plan, "join_enabled", False)
        and plan.segment_count >= 2
        and seg.index > 0
        and seg.task_key in CONTINUITY_TASK_KEYS
        and bool(getattr(seg, "continuity_from_prev", True))
...
def resolve_prev_segment_output(
    ...
    if cached is not None:
        return cached
    if not (plan.continuity_enabled or getattr(plan, "join_enabled", False)):
        return None
    raise ValueError(
