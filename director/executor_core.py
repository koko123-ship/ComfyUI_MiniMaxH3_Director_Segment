...
def execute_director_plan_core(
    ...
    *,
    ...
    if plan.continuity_enabled:
        ...
        reports.append(
            "Segment join: ON — negative-time context "
            f"{snap_context_frames(plan.join_overlap_frames)}f "
            "(frame-0 identity lock, latent never extended, trim 0; "
            "t2v/i2v/fl2v/r2v/v2v/rv2v)."
        )
        if joined:
            reports.append("  Join from prev: #" + ", #".join(str(i) for i in joined))
        if skipped_join:
            reports.append(
                "  Hard cut (per-segment off): #"
                + ", #".join(str(i) for i in skipped_join)
            )
    else:
        reports.append(
    ...
    def _run_one_segment(seg, *, progress_index: int) -> tuple[torch.Tensor, dict[str, Any] | None]:
        ...
        join_active = is_join_active(plan, seg)
        prev_tail = None
        prev_av = None
        prev_audio = None
        prev_end_frame = None
        prev_idx = seg.index - 1
        if continuity_active or join_active:
        ...
        # 段间衔接: negative-time context, mutually exclusive with continuity.
        use_join_context = (
            join_active
            and not i2v_new_anchor
            and (prev_av is not None or prev_tail is not None)
        ...
        if use_motion_context:
            ...
            first_frame = None
        elif use_join_context:
            # Join never extends the latent: sample == align(visible), trim 0.
            context_n = snap_join_context_frames(plan.join_overlap_frames)
            sample_len, _planned_trim = negative_time_frame_budget(
                num_frames, context_n
            )
        else:
            # OFF → context_n=0 → sample_len == official segment length only.
            context_n = 0
            sample_len, _planned_trim = generation_frame_budget(num_frames, 0)
        ...
        if use_motion_context:
            ...
            # 段间衔接: pin prev's true last exported pixel at frame 0 (identity
            # lock), place the rest of the context at negative RoPE times so the
            # latent is never extended. trim=0 → no tail loss at the seam.
            pin_audio = (
                audio_mode != AUDIO_MODE_MUTE
                and (prev_av is not None or prev_audio is not None)
            )
            positive, _trim, _prev_trim = apply_negative_time_context(
                positive,
                latent,
                vae=vae,
                context_length=context_n,
            ...
            task_hint = f"{task_hint} + join context"
            reports.append(
                f"Seg #{seg.index + 1}: 段间衔接 ON — negative-time context "
                f"{context_n}f from seg #{seg.index} "
                f"({'AV latent' if prev_av is not None else 'pixels'}"
                f"{', +audio' if pin_audio else ', video-only'}); "
                f"sample={sample_len}f → export {num_frames}f (trim 0)"
            )
        ...
        if seg.task_key in {"t2v", "i2v", "r2v", "fl2v", "v2v", "rv2v"} and decoded.shape[0] >= 1:
            try:
                # 缩略图预览:降分辨率后再编码,长片段/多段时避免全分辨率 base64 撑爆前端。
                frames_b64 = [
                    tensor_frame_to_jpeg_b64(decoded[i], max_side=360)
