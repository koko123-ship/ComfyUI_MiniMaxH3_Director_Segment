...
def tensor_frame_to_jpeg_b64(
    frame: torch.Tensor,
    *,
    max_side: int = 0,
    quality: int = 88,
) -> str:
    """JPEG base64 of a frame tensor; when max_side > 0, downscale to fit the cap.

    UI 预览用:缩略图只需要几百像素,全分辨率 base64 会在长片段/多段时把前端
    内存与 WebSocket 流量撑爆。其他调用方不传 max_side 时保持原样。
    """
    arr = (frame.detach().cpu().clamp(0, 1).numpy() * 255).astype("uint8")
    img = Image.fromarray(arr)
    if max_side and max_side > 0 and (img.width > max_side or img.height > max_side):
        img = ImageOps.contain(img, (max_side, max_side), Image.LANCZOS)
    buf = io.BytesIO()
