...
class DirectorPlan:
    ...
    # 段间衔接 (join): negative-time context, mutually exclusive with continuity.
    join_enabled: bool = False
    join_overlap_frames: int = 0
    # 稀疏取帧: checked → tail 6 contiguous steps + ~1s single-step anchors out
    # to the chosen reach (scene identity, ~3-4x cheaper); unchecked → dense stack.
    join_sparse_context: bool = False
...
def build_director_plan(
    ...
    *,
    ...
    from .segment_continuity import (
        ...
        resolve_join_settings,
        resolve_join_sparse_context,
    ...
    join_enabled, join_overlap = resolve_join_settings(
        timeline, segment_count=len(segments)
    )
    join_sparse = bool(join_enabled) and resolve_join_sparse_context(timeline)
    if continuity_enabled and join_enabled:
        raise ValueError(
            "MiniMax H3 Director: 「段间引导」与「段间衔接」不能同时开启，"
            "请在时间轴输出栏只保留一个总开关。"
        )
    ...
    return DirectorPlan(
        ...
        join_overlap_frames=join_overlap,
        join_sparse_context=join_sparse,
...
def plan_summary(plan: DirectorPlan) -> str:
    ...
    if mode in ("gen_blank", "gen_image", "prompt_batch", "image_batch", "fl2v"):
        ...
        if plan.continuity_enabled:
            ...
            lines.append(
                f"Segment join: ON (negative-time context {plan.join_overlap_frames}f, trim 0)"
            )
            if joined:
                lines.append("  Join from prev: #" + ", #".join(str(i) for i in joined))
            if skipped_join:
                lines.append(
                    "  Hard cut (per-segment off): #"
    ...
    if plan.continuity_enabled:
        ...
        lines.append(
            f"Segment join: ON (negative-time context {plan.join_overlap_frames}f "
            "→ frame-0 identity lock, trim 0; t2v/i2v/fl2v/r2v/v2v/rv2v)"
        )
        if joined:
            lines.append(
                "  Join from prev: #"
                + ", #".join(str(i) for i in joined)
            )
        if skipped_join:
            lines.append(
                "  Hard cut (per-segment off): #"
